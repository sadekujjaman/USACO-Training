...
//		PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter("src/test.out")));
		...
		InputReader in = new InputReader(new FileInputStream("runround.in"));
...
*/
	...
	private static boolean isRunAroundNum(long n) {
		...
		for(int i = 0; i < len; i++) {
			...
			visited1[arr[i + 1]] = true;
		}

		int st = 1;
		int start = arr[st];
		while(true) {
			if(visited[st] && arr[st] == start) {
			...
			if(st + val <= len) {
				...
				int v = (st + (len - st) + val - (len - st)) % len;
				if(v == 0) {
					st = len;
				}
				else {
