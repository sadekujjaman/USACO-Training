...
//		PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter("src/test.out")));
		...
		InputReader in = new InputReader(new FileInputStream("stall4.in"));
...
*/
		...
		int m = in.nextInt();
		int node = n + m + 2;
		List<Edge>[] adjList = createGraph(node);
		for (int i = 1; i <= n; i++) {
			int a = in.nextInt();
			for(int j = 0; j < a; j++) {
				int v = in.nextInt();
				addEdge(adjList, i, v + n, 1);
			}
			addEdge(adjList, 0, i, 1);
		}
		
		for(int i = n + 1; i < node - 1; i++) {
			addEdge(adjList, i, node - 1, 1);
		}
		
		
		
		
		long ans = maxFlow(adjList, 0, node - 1);
